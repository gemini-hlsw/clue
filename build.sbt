...
lazy val genTests = project
  ...
  .settings(
    ...
    scalafixTestkitOutputSourceDirectories :=
      sourceDirectories.in(genOutput, Compile).value,
    scalafixTestkitInputSourceDirectories :=
      sourceDirectories.in(genInput, Compile).value,
    scalafixTestkitInputClasspath :=
      fullClasspath.in(genInput, Compile).value :+
        Attributed.blank(semanticdbTargetRoot.in(genInput, Compile).value),
    compile.in(Compile) := compile
      .in(Compile)
      .dependsOn(compile.in(genInput, Compile))
      .value
  )
  .dependsOn(genRules)
