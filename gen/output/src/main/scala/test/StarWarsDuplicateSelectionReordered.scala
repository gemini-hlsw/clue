...
object StarWarsDuplicateSelectionReordered extends GraphQLOperation[StarWars] {
  ...
  object Data {
    ...
    object Hero {
      case class Friends(val id: String, val name: Option[String] = None)
      object Friends {
  ...
  class ClientAppliedFP[F[_], P](val client: clue.FetchClientWithPars[F, P, StarWars]) { def query(ep: Episode, modParams: P => P = identity) = client.request(StarWarsDuplicateSelectionReordered).withDescriptor("StarWarsDuplicateSelectionReordered").withInput(Variables(ep), modParams) }
}
// format: on
