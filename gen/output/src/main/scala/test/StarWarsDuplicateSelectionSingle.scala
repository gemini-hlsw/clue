...
object StarWarsDuplicateSelectionSingle extends GraphQLOperation[StarWars] {
  ...
  class ClientAppliedFP[F[_], P](val client: clue.FetchClientWithPars[F, P, StarWars]) { def query(ep: Episode, modParams: P => P = identity) = client.request(StarWarsDuplicateSelectionSingle).withDescriptor("StarWarsDuplicateSelectionSingle").withInput(Variables(ep), modParams) }
}
// format: on
