...
object LucumaQuery3 extends GraphQLOperation[LucumaODB] {
  ...
  class ClientAppliedFP[F[_], P](val client: clue.FetchClientWithPars[F, P, LucumaODB]) { def query(modParams: P => P = identity) = client.request(LucumaQuery3).withDescriptor("LucumaQuery3").withInput(Variables(), modParams) }
}
// format: on
