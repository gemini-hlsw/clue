...
object Wrapper extends Something {
  ...
  object StarWarsQuery2 extends GraphQLOperation[StarWars] {
    ...
    class ClientAppliedFP[F[_], P](val client: clue.FetchClientWithPars[F, P, StarWars]) { def query(charId: String, modParams: P => P = identity) = client.request(StarWarsQuery2).withDescriptor("StarWarsQuery2").withInput(Variables(charId), modParams) }
  }
}
// format: on
