...
object LucumaQuery2 extends GraphQLOperation[LucumaODB] {
  ...
  class ClientAppliedFP[F[_], P](val client: FetchClientWithPars[F, P, LucumaODB]) { def query(modParams: P => P = identity) = client.request(LucumaQuery2).withDescriptor("LucumaQuery2").withInput(Variables(), modParams) }
}
// format: on
