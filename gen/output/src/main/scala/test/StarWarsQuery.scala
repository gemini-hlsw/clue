...
object StarWarsQuery extends GraphQLOperation[StarWars] {
  ...
  class ClientAppliedFP[F[_], P](val client: clue.FetchClientWithPars[F, P, StarWars]) { def query(charId: String, modParams: P => P = identity) = client.request(StarWarsQuery).withDescriptor("StarWarsQuery").withInput(Variables(charId), modParams) }
}
// format: on
