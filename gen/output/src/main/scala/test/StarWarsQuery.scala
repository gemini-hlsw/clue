...
object StarWarsQuery extends GraphQLOperation[StarWars] {
  ...
  object Data {
    ...
    object Character {
      ...
      case class Droid(override val id: String, override val name: Option[String] = None, override val friends: Option[List[Data.Character.Friends]] = None, @deprecated("reason: Use 'functions' instead") val primaryFunction: Option[String] = None) extends Character()
      object Droid {
  ...
  class ClientAppliedFP[F[_], P](val client: clue.FetchClientWithPars[F, P, StarWars]) { def query(charId: String, modParams: P => P = identity) = client.request(StarWarsQuery).withInput(Variables(charId), modParams) }
}
// format: on
