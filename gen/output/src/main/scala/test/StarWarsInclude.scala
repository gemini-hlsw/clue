...
object StarWarsInclude extends GraphQLOperation[StarWars] {
  ...
  class ClientAppliedFP[F[_], P](val client: clue.FetchClientWithPars[F, P, StarWars]) { def query(humanId: String, skipId: Boolean, withName: Boolean, modParams: P => P = identity) = client.request(StarWarsInclude).withInput(Variables(humanId, skipId, withName), modParams) }
}
// format: on
