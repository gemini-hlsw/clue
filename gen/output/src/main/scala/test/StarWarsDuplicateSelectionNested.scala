...
object StarWarsDuplicateSelectionNested extends GraphQLOperation[StarWars] {
  ...
  object Data {
    ...
    object Hero {
      ...
      object Friends {
        val name: monocle.Iso[Data.Hero.Friends, Option[String]] = monocle.Focus[Data.Hero.Friends](_.name)
  ...
  class ClientAppliedFP[F[_], P](val client: clue.FetchClientWithPars[F, P, StarWars]) { def query(ep: Episode, modParams: P => P = identity) = client.request(StarWarsDuplicateSelectionNested).withDescriptor("StarWarsDuplicateSelectionNested").withInput(Variables(ep), modParams) }
}
// format: on
