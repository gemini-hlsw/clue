...
object StarWarsDuplicateSelectionSpliced extends GraphQLOperation[StarWars] {
  ...
  object Data {
    sealed trait Hero { val friends: Option[List[StarWarsSubquery.Data]] }
    object Hero {
      case class Human(override val friends: Option[List[StarWarsSubquery.Data]] = None, val homePlanet: Option[String] = None) extends Hero()
      object Human {
        val friends: monocle.Lens[Data.Hero.Human, Option[List[StarWarsSubquery.Data]]] = monocle.macros.GenLens[Data.Hero.Human](_.friends)
      ...
      case class Other(override val friends: Option[List[StarWarsSubquery.Data]] = None) extends Hero()
      object Other {
        val friends: monocle.Iso[Data.Hero.Other, Option[List[StarWarsSubquery.Data]]] = monocle.Focus[Data.Hero.Other](_.friends)
      ...
      val friends: monocle.Lens[Data.Hero, Option[List[StarWarsSubquery.Data]]] = monocle.Lens[Data.Hero, Option[List[StarWarsSubquery.Data]]](_.friends) {
        v => _ match {
  ...
  class ClientAppliedFP[F[_], P](val client: clue.FetchClientWithPars[F, P, StarWars]) { def query(ep: Episode, modParams: P => P = identity) = client.request(StarWarsDuplicateSelectionSpliced).withDescriptor("StarWarsDuplicateSelectionSpliced").withInput(Variables(ep), modParams) }
}
// format: on
