      ...
      .flatMap: (connection, release) =>
        for {
          ...
          releaseNow    = releaseState
                            ...
                            .flatMap(released => if released then release else Concurrent[F].unit)
          notifyClose   = (event: CloseEvent) =>
                            closeState
                              .modify(closed => true -> !closed)
                              .flatMap(closed =>
                                if closed then handler.onClose(connectionId, event)
                                else Concurrent[F].unit
                              )
          canceledClose = notifyClose(
